...
*/
#define BLOCK_PUSH_FACTOR 0.01f
#define MARGIN 0.0f // margin thêm vào các va chạm
    ...
    float& t, float& nx, float& ny)
{
    ...
    //
    // Broad-phase test with margin
    //
    float bl = dx > 0 ? ml - MARGIN : ml + dx - MARGIN;
    float bt = dy > 0 ? mt - MARGIN : mt + dy - MARGIN;
    float br = dx > 0 ? mr + dx + MARGIN : mr + MARGIN;
    float bb = dy > 0 ? mb + dy + MARGIN : mb + MARGIN;
...
/*
    Calculate potential collisions with the list of collidable objects

    coObjects: the list of collidable objects
...
void CCollision::Process(LPGAMEOBJECT objSrc, DWORD dt, vector<LPGAMEOBJECT>* coObjects)
{
    ...
    DebugOut(L"Number of collision events: %d\n", coEvents.size());
    // No collision detected
    if (coEvents.size() == 0) {
        objSrc->OnNoCollision(dt);
		// debug log number of collision events
    }
    else {
